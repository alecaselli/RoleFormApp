...
/*
    ...
    public void aggiungiDescrizione(StringBuffer note){
        this.descrizione.append(note);
    }
    ...
    public void aggiornaPuntiFeritaMax(int val){
        this.puntiFeritaMax += val;
    }
    ...
    private void aggiornaPuntiStat(int val){
        this.puntiStat += val;
    }

    private void aggiornaModCompetenza(int livello){
        if((livello % 4) == 0)
            this.modCompetenza += 2;
    }

    private void aggiornaNDadi(){
        this.nDadi ++ ;
    }
    ...
    public void aggiungiEquipaggiato(List<Equipaggiamento> nuovo){
            ...
            for (Equipaggiamento equi : nuovo){
                if(equi.getTipo().matches("arma ...")){
    ...
    public void inizializzazionePG(){
        ...
        for(CaratteristicaBase elementoR : this.razza.getCaratteristicaBaseList()){
            for(Caratteristica elementoC : this.caratteristicaList){
                if(elementoC.getNome().compareToIgnoreCase(elementoR.getNome())==0)
                    elementoC.aggiornaValoreBase(elementoR.getValore());
            }
        }
        ...
        this.aggiungiDescrizione(this.classe.getDescrizionePrivilegiPoteri());
        this.aggiungiDescrizione(this.classe.getCompetenza());
